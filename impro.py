# module "impro.py"
# Generic functions to analyse data in terms of IGs or more generaly in terms of timestamps,
# as obtained in Collective Free Improvisation experiments
#
...
def datalists(participant):
    
    with open(participant) as json_file:
        participant = json.load(json_file)

    timelist1 = []
    for j in participant['musicAnalysis']:
        for key, value in j['timestamp'].items():
            value_int = int(value)
        for key, value in j['value'].items():
            mark_int = int(value)
            timelist1.append((value_int, mark_int))
...
#        print(i, ind_y[ind])
    ...
    found = numpy.sort(found)   # added 2022-03-30, mmaybe useless
...
# function to count the (total) number of "simultaneous" IGs in a trio
#
...
def count_nb_duo(data, threshold=5, tau=2, causal=0, do_clean=1, do_fraction=0):
    ''' function to count the (total) number of "simultaneous" IGs in a trio
